...
function generatePauseTime(time, pos, startTime, pauseID) {
    ...
    let pauseTimeElement = document.getElementById(
        `pauseTimeProgress${pauseID}`
    );
        ...
        if (pauseTimeElement) {
            if (i == data['pauseMode']) {
                ...
                if (i < data['pauseMode']) {
                    ...
                    data['activeMatchNumber'] < -1 &&
                    data['activeMatchNumber'] > -100
                ) {
                    if (
                        positions[i] > data['activeMatchNumber'] ||
                        positions[i] < -99
                    ) {
                        pauseTimeElement.value = 999999999;
                    } else {
...
function updatePauseTime() {
    if (intervalActivated == true) {
        ...
        let pauseTimeElement = document.getElementById(
            `pauseTimeProgress${data['pauseMode']}`
        );
...
function KOMatchTable() {
    ...
    if ('KOMatches' in data && data['KOMatches'] != null) {
        if (
            data['pauseBeforeKOMatches'] != null &&
            data['pauseBeforeKOMatches'] != '0'
        ) {
            var requestedtime = parseInt(data['pauseBeforeKOMatches']);
            KOMatchesTime = generatePauseTime(
                requestedtime,
                12,
                KOMatchesTime,
                0
        ...
        if (KOMatches != null) {
            ...
            finalMatchesTime = new Date(
                KOMatchesTime.getTime() + y * data['timeIntervalFM'] * 60000
            );
        }
    }
}
    ...
    if ('finalMatches' in data && data['finalMatches'] != null) {
        if (data['pauseBeforeFM'] != null && data['pauseBeforeFM'] != '0') {
            ...
            if (data['KOMatches'] != null) {
                finalMatchesTime = new Date(
                    generatePauseTime(requestedtime, -103, finalMatchesTime, 1)
                );
            } else {
                finalMatchesTime = new Date(
                    generatePauseTime(requestedtime, 12, finalMatchesTime, 1)
                );
            }
            finalMatchesTime = new Date(
        ...
        finalMatchesSliced.forEach((match) => {
            ...
            if (i == totalMatchNumber) {
                gameName = 'Finale';
                if (
                    data['halfTimePause'] != null &&
                    data['halfTimePause'] != '0'
                ) {
                    finalMatchesTime = new Date(
                        generatePauseTime(
                            parseInt(data['halfTimePause']),
                            -4,
                            finalMatchesTime,
                            3
                        )
                ...
                if (
                    data['pauseBeforeTheFinalMatch'] != null &&
                    data['pauseBeforeTheFinalMatch'] != '0'
                ) {
                    finalMatchesTime = new Date(
                        generatePauseTime(
                            parseInt(data['pauseBeforeTheFinalMatch']),
                            -3,
                            finalMatchesTime,
                            2
                        )
